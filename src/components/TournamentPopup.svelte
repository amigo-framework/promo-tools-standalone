<script lang="ts">
  ...
  $: primaryButtonLabel = mode === 'finished' ? tr('tournamentFinishedAcknowledgeButton') : mode === 'active' ? tr('continue') : tr('start');
  ...
  >
    ...
    >
      ...
      <div class="promo-image-overlay-content">
        <div id="popup-title" class="promo-title" class:finished={mode === 'finished'}>{title}</div>
        {#if mode === 'finished'}
...
.promo-title{position:absolute;left:50%;top:43%;width:calc(100% - 40px);margin:0;transform:translateX(-50%);text-align:center;font-size:34px;font-weight:1000;line-height:1.05;background:linear-gradient(180deg,#ffff00,#ffa500);-webkit-background-clip:text;background-clip:text;-webkit-text-fill-color:transparent;-webkit-text-stroke:2px #000000}
.promo-title.finished{font-size:23px;top:205px}
...
.promo-actions{position:absolute;left:50%;right:auto;bottom:37px;display:flex;justify-content:center;align-items:center;gap:8px;padding:0 7px;transform:translateX(-50%);pointer-events:auto}
